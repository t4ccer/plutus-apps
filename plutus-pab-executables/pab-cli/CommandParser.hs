...
import Types (AppError (ChainIndexPortError, NodePortError, PABDBPoolSizeError, PABExeNotExecutableError, PABExeNotProvidedError, PABPortError, PABResumeFromBlockIdNotSpecifiedError, PABResumeFromSlotNotANumberError, PABResumeFromSlotNotSpecifiedError, PABRollbackHistoryNotNumberError, UnspecifiedNodeNetworkError, WalletPortError),
              ...
              NodeDirectory (NodeDirectory), NodeOpts (NodeOpts), NodePort (NodePort), PABDirectory (PABDirectory),
              PABExe (PABExe), PABOpts (PABOpts), PABPort (PABPort), WalletPort (WalletPort))
...
argsToAppOpts args = do
    ...
    then
        ...
        then do
            ...
            pabOpts <- parsePABOpts args >>= maybe (throwM PABExeNotProvidedError) pure
...
parsePABOpts :: (MonadIO m, MonadThrow m) => Arguments -> m (Maybe PABOpts)
parsePABOpts args = do
    pabExeM <- parsePABExe args
    case pabExeM of
      Nothing -> pure Nothing
      Just pabExe -> do
          fmap Just
          $ PABOpts pabExe <$> parsePABOutputDir args
                           <*> parsePABPort args
...
parsePABExe args = do
    ...
    then do
        ...
        liftIO (doesFileExist pe)
            >>= \fileExists -> unless fileExists $ throwM PABExeNotProvidedError
...
parsePABOutputDir :: (MonadIO m) => Arguments -> m PABDirectory
parsePABOutputDir args =
    PABDirectory <$> liftIO (args `getArgOrExit` longOption "pab-dir")
...
parseNodeOutputDir :: (MonadIO m) => String -> Arguments -> m NodeDirectory
parseNodeOutputDir subdir args = do
    nodeDir <- liftIO (args `getArgOrExit` longOption "node-dir")
    pure $ NodeDirectory $ nodeDir </> subdir
